...
const { readAuthSharedStoreLocation } = require("./openclaw-state-era");
...
const createOpenclawChannelSync = ({
  ...
  const reconcileBootConfigInner = async ({
    ...
    //
    // Two gates keep the forced migration honest:
    //   era  - only builds whose runtime demands the sqlite auth store
    //          (>= 2026.9.1, the shared-store era documented in
    //          openclaw-state-era.js) may trigger it. On a downgrade to a
    //          pre-era build the JSON store IS the correct credential home,
    //          and unattended doctor runs are forbidden inside rollback
    //          stabilization, so a populated file alone must never force a
    //          migration there.
    //   flag - the auth.sharedStore machine-state row is the sole authority
    //          for which store the runtime reads; once it says state-db, a
    //          leftover auth-profiles.json is inert debris, not a pending
    //          migration, and must not refire doctor on every boot.
    const legacyAuthStorePresent = hasCredentialBearingLegacyAuthStore();
    const installedCore = coreVersionOf(installedVersion);
    const authStoreEraApplies =
      build?.source === "dev" ||
      (installedCore !== null &&
        compareVersionParts(installedCore, "2026.9.1") >= 0);
    const legacyAuthStoreMigrationNeeded =
      legacyAuthStorePresent &&
      authStoreEraApplies &&
      readAuthSharedStoreLocation({ fsModule, openclawDir, logger }) !==
        "state-db";
    if (legacyAuthStoreMigrationNeeded) {
      log(
        "config-gate: credential-bearing legacy auth profile store detected - doctor --fix will run even though the config is current",
      );
    } else if (legacyAuthStorePresent) {
      log(
        "config-gate: legacy auth profile store present but migration not required on this build (pre-2026.9.1 era, or the shared store already owns auth) - left untouched",
      );
    }
    ...
    if (
      ...
      !state.gatewayHold &&
      !force &&
      !legacyAuthStoreMigrationNeeded
    ) {
      resolvePendingRun();
    ...
    // Fail CLOSED when the auth-store migration was the point and did not
    // complete - doctor failed, or reported success while the legacy store
    // still owns credentials (file populated, flag not flipped). Recording
    // success here would launch a gateway whose agent runs all fail with
    // AUTH_PROFILE_MIGRATION_REQUIRED, and an ok attempt record would disarm
    // the cross-boot re-attempt gate so every restart re-ran the sized
    // doctor budget forever.
    const legacyAuthStoreRemaining =
      legacyAuthStoreMigrationNeeded &&
    ...
    const doctorNote = legacyAuthStoreRemaining
      ? doctorOutcome?.ok === true
        ? "doctor completed but the legacy auth profile store still owns credentials"
        : "legacy auth profile store migration did not complete"
